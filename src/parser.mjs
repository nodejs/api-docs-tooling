...
const createParser = () => {
    ...
    apiDocProcessor.use(() => {
      ...
      return tree => {
        visit(tree, createQueries.UNIST.isHeadingNode, (headingNode, index) => {
          ...
          const parsedSection = apiDocProcessor.stringify(
            subtree,
            resolvedApiDoc
          );
          ...
          const parsedApiEntryMetadata = apiEntryMetadata.create(
            ...
            new VFile({ ...resolvedApiDoc, value: parsedSection })
          );
  ...
  const parseApiDocs = async apiDocs => {
    ...
    // Since each individual API doc has a collection of Metadata entries,
    // we flatten the resulting array to combine all entries into a single collection
